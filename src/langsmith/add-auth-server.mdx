...
You'll keep the same [`Auth`](/langsmith/smith-python-sdk#langgraph_sdk.auth.Auth) object and [resource-level access control](/langsmith/auth#single-owner-resources), but upgrade authentication to use Supabase as your identity provider. While Supabase is used in this tutorial, the concepts apply to any OAuth2 provider. You'll learn how to:
...
In the previous tutorials, you used the [`Auth`](/langsmith/smith-python-sdk#langgraph_sdk.auth.Auth) object to [validate hard-coded tokens](/langsmith/set-up-custom-auth) and [add resource ownership](/langsmith/resource-auth).

Now you'll upgrade your authentication to validate real JWT tokens from Supabase. The main changes will all be in the [`@auth.authenticate`](/langsmith/smith-python-sdk#langgraph_sdk.auth.Auth.authenticate) decorated function:
...
3. Customize your handlers and setup further after reading the [reference docs](/langsmith/smith-python-sdk#langgraph_sdk.auth.Auth).

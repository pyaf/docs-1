...
An application must be configured with a [configuration file](/langsmith/cli#configuration-file) in order to be deployed to LangSmith Platform (or to be self-hosted). This how-to guide discusses the basic steps to set up a JavaScript application for deployment using `package.json` to specify project dependencies.
...
const workflow = new StateGraph(MessagesAnnotation)
  ...
  .addConditionalEdges(
    ...
    // Required for conditional edges to properly render the graph in the Debugger
    ["tools", "__end__"]
  )

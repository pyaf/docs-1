...
Custom state extends the default agent state with additional fields. The @[`state_schema`] parameter on @[`create_agent`] defines your custom state:
...
<Note>
    Custom state is defined by creating a class that extends @[`AgentState`] and passing it to @[`create_agent`]'s @[`state_schema`] parameter.
</Note>

...
Because `create_agent` is built on [LangGraph](/oss/langgraph), you automatically get built in support
for long running, reliable agents with:
...
<Note>
    Content block support is currently ony available for the following integrations:
...
The new `content_blocks` property is built to provide unified access to modern LLM features across providers:

...
**Deterministic guardrails** use rule-based logic like regex patterns, keyword matching, or explicit checks. They're fast, predictable, and cost-effective, but may miss nuanced violations.

**Model-based guardrails** use LLMs or classifiers to evaluate content with semantic understanding. They catch subtle issues that rules miss, but are slower and more expensive.

LangChain provides both built-in guardrails (PII detection, human-in-the-loop) and a flexible middleware system for building custom guardrails using either approach.
...
# In your application, present the pending action to a human reviewer
# Then resume execution with approval
result = agent.invoke(result, command={"command": "resume"})
```
:::
...
// In your application, present the pending action to a human reviewer
// Then resume execution with approval
result = await agent.invoke(result, { command: { command: "resume" } });
```
:::
...
<Note>
When using `jump_to: "end"` to exit early, ensure your middleware hook is configured to support jumps. In Python, use the `@hook_config(can_jump_to=["end"])` decorator. In JavaScript, jumps are supported by default.
</Note>

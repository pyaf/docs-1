...
:::python
```python Conversation history wrap
...
:::js
```typescript Conversation history wrap
...
<CodeGroup>
    ...
    ```python Stream tool calls, reasoning, and other content wrap
...
:::js
```typescript Binding user tools highlight={17} wrap
...
<AccordionGroup>
    <Accordion title="Tool execution loop" icon="arrow-rotate-right">
        ...
        ```python Tool execution loop wrap
    ...
    <Accordion title="Forcing tool calls" icon="asterisk">
        ...
        <CodeGroup>
            ...
            ```python Force use of specific tools wrap
    ...
    <Accordion title="Parallel tool calls" icon="layer-group">
        ...
        ```python Parallel tool calls wrap
        model_with_tools = model.bind_tools([get_weather])
...
<AccordionGroup>
    <Accordion title="Tool execution loop" icon="arrow-rotate-right">
        ...
        ```typescript Tool execution loop wrap
        //  Bind (potentially multiple) tools to the model
    ...
    <Accordion title="Forcing tool calls" icon="asterisk">
        ...
        <CodeGroup>
            ```typescript Force use of any tool wrap
            const modelWithTools = model.bindTools([tool_1], { toolChoice: "any" })
            ```

            ```typescript Force use of specific tools wrap
    ...
    <Accordion title="Parallel tool calls" icon="layer-group">
        ...
        ```typescript Parallel tool calls wrap
    ...
    <Accordion title="Streaming tool calls" icon="rss">
        ...
        ```typescript Accumulate tool calls wrap
        let full: AIMessageChunk | null = null
...
<Accordion title="Example: Nested structures">
    ...
    :::python
    <CodeGroup>
        ```python Pydantic BaseModel wrap
        from pydantic import BaseModel, Field
...
:::python
<CodeGroup>
    ```python Stream reasoning output wrap
    for chunk in model.stream("Why do parrots have colorful feathers?"):
        reasoning_steps = [r for r in chunk.content_blocks if r["type"] == "reasoning"]
        print(reasoning_steps if reasoning_steps else chunk.text)
    ```

    ```python Complete reasoning output wrap
    response = model.invoke("Why do parrots have colorful feathers?")
...
:::js
<CodeGroup>
    ```typescript Stream reasoning output wrap
    ...
    ```typescript Complete reasoning output wrap
    const response = await model.invoke("Why do parrots have colorful feathers?");
...
<Accordion title="Initialize and use a rate limiter" icon="gauge-high">
    ...
    ```python Define a rate limiter highlight={12} wrap

...
:::python
To trim message history in an agent, use the `@before_model` middleware decorator with the [`trim_messages`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.utils.trim_messages.html) function:
...
@before_model
def trim_message_history(state: AgentState, runtime: Runtime) -> dict[str, list[BaseMessage]] | None:
    """Trim messages before every model call to stay within context limits."""
    ...
    # Return None if no trimming needed
...
### In middleware (message processing)
...
```python
from langchain_core.messages.utils import trim_messages, count_tokens_approximately
from langchain.messages import BaseMessage, RemoveMessage
from langgraph.checkpoint.memory import InMemorySaver
from langchain.agents import create_agent, AgentState
...
@before_model
def trim_message_history(state: AgentState, runtime: Runtime) -> dict | None:
...
agent = create_agent(
    model="openai:gpt-5-nano",
    tools=[],
    middleware=[trim_message_history, validate_response],
    checkpointer=InMemorySaver(),
)
```
:::
...
const agent = createAgent({
    model: "openai:gpt-5-nano",
    tools: [],
    middleware: [trimMessageHistory, validateResponse],
});
```
:::

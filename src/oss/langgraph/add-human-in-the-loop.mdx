...
:::js
```typescript {highlight={1,5,31}} wrap
...
```
  ...
  :::js
  ```typescript {highlight={12,43}} wrap
...
:::python
```python title="Wrapper that adds human-in-the-loop to any tool" {highlight={37}} wrap
...
:::python
```python {highlight={4,14,16,24}} wrap
from langgraph.checkpoint.memory import InMemorySaver
from langchain.agents import create_agent
...
### Use static interrupts in Studio

You can use [Studio](/langsmith/studio) to debug your graph. You can set static breakpoints in the UI and then run the graph. You can also use the UI to inspect the graph state at any point in the execution.

![image](/oss/images/static-interrupt.png)

Studio is free with [locally deployed applications](/oss/langgraph/local-server) using `langgraph dev`.
:::

...
- **[LangSmith](#langsmith)** - Install the LangSmith platform with UI for observability, tracing, and evaluation. Your applications are deployed separately using your own CI/CD.
- **[LangSmith with Deployment](#langsmith-with-deployment)** - Full enterprise platform with both observability features and UI-based deployment management for your agents. Requires LangSmith to be installed first.
- **[Standalone Server](#standalone-server)** - Lightweight option to deploy individual LangGraph Servers without the LangSmith UI. Ideal for microservice-style deployments.

| Feature | **LangSmith** | **LangSmith + Deployment** | **Standalone Server** |
|---------|---------------|---------------------------|---------------------|
| **LangSmith UI** | ✅ Yes | ✅ Yes | ❌ No |
| **Observability & Tracing** | ✅ Yes | ✅ Yes | Optional (integrate with LangSmith) |
| **Deploy agents via UI** | ❌ No | ✅ Yes | ❌ No (manual deployment) |
| **Control plane** | ❌ No | ✅ Yes | ❌ No |
| **Data plane** | ❌ No | ✅ Yes | ✅ Yes (just servers) |
| **Kubernetes required** | Recommended | Required | Optional |
| **Setup complexity** | Medium | High | Low |
| **Best for** | Monitoring existing apps | Enterprise agent platform | Microservice-style deployments |
...
### Best for
- Teams who need self-hosted monitoring and evaluation
- Running the LangSmith app without deploying agents/graphs through a UI
- Organizations that deploy agents separately using their own CI/CD
...
- Organizations wanting deployment management alongside observability

### What you get

Everything from [LangSmith](#langsmith), plus:
...
### Best for
- Teams who want to run one or a few agents as independent services
- Integrating LangGraph Servers as microservices into existing systems
- Full control over scaling, deployment, and CI/CD pipelines
- Organizations that don't need the LangSmith UI

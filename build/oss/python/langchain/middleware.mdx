...
You can extend the agent's state with custom properties using the `state_schema` parameter on `create_agent`. Middleware can then access these custom state fields:
...
agent = create_agent(
    ...
    state_schema=CustomState,  # Define custom state here
    tools=[...],
)

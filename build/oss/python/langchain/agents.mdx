...
Custom state schemas must extend [`AgentState`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.AgentState) as a `TypedDict`. Pass them to [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) using the `state_schema` parameter:
...
agent = create_agent(
    model,
    tools=tools,
    state_schema=CustomAgentState  # Pass custom state schema here
)

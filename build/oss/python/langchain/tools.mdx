...
Use `ToolRuntime` to access all runtime information in a single parameter. Simply add `tool_runtime: ToolRuntime` to your tool signature, and it will be automatically injected without being exposed to the LLM.
...
def get_user_preference(
    pref_name: str,
    tool_runtime: ToolRuntime  # ToolRuntime parameter is not visible to the model
) -> str:
    """Get a user preference value."""
    preferences = tool_runtime.state.get("user_preferences", {})
...
Access immutable configuration and contextual data like user IDs, session details, or application-specific configuration through `tool_runtime.context`.
...
@tool
def get_account_info(tool_runtime: ToolRuntime[UserContext]) -> str:
...
Access persistent data across conversations using the store. The store is accessed via `tool_runtime.store` and allows you to save and retrieve user-specific or application-specific data.
...
# Access memory
@tool
def get_user_info(user_id: str, tool_runtime: ToolRuntime) -> str:
    """Look up user info."""
...
# Update memory
@tool
def save_user_info(user_id: str, user_info: dict[str, Any], tool_runtime: ToolRuntime) -> str:
    """Save user info."""
    store = tool_runtime.store
...
Stream custom updates from tools as they execute using `tool_runtime.stream_writer`. This is useful for providing real-time feedback to users about what a tool is doing.

```python wrap
from langchain.tools import tool, ToolRuntime

@tool
def get_weather(city: str, tool_runtime: ToolRuntime) -> str:
...
<Note>
If you use `tool_runtime.stream_writer` inside your tool, the tool must be invoked within a LangGraph execution context. See [Streaming](/oss/python/langchain/streaming) for more details.
</Note>

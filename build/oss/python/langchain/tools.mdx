...
Tools can access and update the store through `get_store()`:
...
Stream custom updates from tools as they execute using `get_stream_writer()`. This is useful for providing real-time feedback to users about what a tool is doing.
...
<Note>
If you use `get_stream_writer()` inside your tool, the tool must be invoked within a LangGraph execution context. See [Streaming](/oss/python/langchain/streaming) for more details.
</Note>

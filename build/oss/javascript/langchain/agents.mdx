...
Agents are built from three core components: models, tools, and system prompts. For advanced control over agent behavior, you can use [middleware](/oss/javascript/langchain/middleware) to customize how your agent processes information, manages state, and responds to users.
...
Pre-model hook is middleware that processes state before the model is called. Use cases include message trimming, summarization, and context injection. For comprehensive middleware documentation, see [Middleware](/oss/javascript/langchain/middleware).
...
After model hook is middleware that processes the model's response before tool execution. Use cases include validation, guardrails, or other post-processing. For comprehensive middleware documentation, see [Middleware](/oss/javascript/langchain/middleware).

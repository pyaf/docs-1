...
workflow.addNode(
    "searchDocumentation",
    searchDocumentation,
    { retryPolicy: new RetryPolicy({ maxAttempts: 3, initialInterval: 1.0 }) }
);
```
...
async function classifyIntent(state: EmailAgentStateType) {
    ...
    let goto: "searchDocumentation" | "humanReview" | "draftResponse" | "bugTracking";
...
workflow.addNode("readEmail", readEmail);
...
workflow.addNode("bugTracking", bugTracking);
workflow.addNode("draftResponse", draftResponse);
workflow.addNode("humanReview", humanReview);
workflow.addNode("sendReply", sendReply);

// Add only the essential edges
workflow.addEdge(START, "readEmail");
workflow.addEdge("readEmail", "classifyIntent");
workflow.addEdge("sendReply", END);

...
It's designed to work hand-in-hand with [LangChain v1](/oss/javascript/javascript/releases/langchain-v1) (whose `createAgent` is built on LangGraph) so you can start high-level and drop down to granular control when needed.
...
* For information on the new `createAgent` API, see the [LangChain v1 release notes](/oss/javascript/javascript/releases/langchain-v1#createagent).
* For information on migrating from `createReactAgent` to `createAgent`, see the [LangChain v1 migration guide](/oss/javascript/javascript/migrate/langchain-v1#createagent).
...
See the [migration guide](/oss/javascript/javascript/migrate/langgraph-v1#stream-encoding) for more information.
